...
class Reader:
    ...
    def __OCR_image(self, img):
        if _platform == "win32":
            ...
            params.append(8)
            cv2.imwrite("tesseract/OCR_candidate.png", img, params)

            # Secondly run tesseract.exe and process the image
            args = ('tesseract/tesseract.exe', 'tesseract/OCR_candidate.png', 'tesseract/out', 'nobatch', 'letters')
            popen = subprocess.Popen(args, stdout=subprocess.PIPE)
            popen.wait()
            output = popen.stdout.read()

            # Lastly open and read the prepared recognized string
            f = open('tesseract/out.txt')
